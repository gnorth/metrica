...
function InsightsView({ entries, stats }: { entries: Entry[]; stats: { avg: string; avgDuration: number; totalOrgasms: number; weekCount: number; eveningShare: number } }) {
  ...
  const heatDays = Array.from({length:35},(_,index) => { const date=new Date(); date.setDate(date.getDate()-(34-index)); return {date, count:entries.filter(item=>new Date(item.createdAt).toDateString()===date.toDateString()).length}; });
  ...
  return <>
    ...
    <article className="analytics-card streak-card"><div className="analytics-head"><div><span className="section-kicker">Ритм</span><h3>Останні 5 тижнів</h3></div><span className="streak-count"><Flame/>2 дні</span></div><div className="heatmap">{heatDays.map((item,index)=><span key={index} className={item.count?`level-${Math.min(item.count,3)}`:''} title={`${item.date.toLocaleDateString('uk-UA')}: ${item.count}`}/>)}</div><div className="heatmap-foot"><span>5 тижнів тому</span><span>сьогодні</span></div><p>Регулярність — це спостереження, не обов’язок.</p></article></section>
